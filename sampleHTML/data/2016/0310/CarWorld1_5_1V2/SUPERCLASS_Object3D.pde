...
class Object3D {
  ...
  boolean on_drag, on_collision, stopped,isWork=true;
  ...
  boolean mouseop(float mx, float my, float mz, boolean pressed){
    if(enable_drag){
      if( over(mx, my, mz) && pressed && !on_drag ){
  ...
  void update(){
    if(x>box.x2 || y>box.y2|| z>box.z2) isWork=false;
    else isWork=true;
    if(isWork)
    {
