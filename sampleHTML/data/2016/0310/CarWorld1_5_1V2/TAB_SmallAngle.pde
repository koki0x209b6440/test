...
PVector handleturn(PVector r, PVector w)
{
  ...
  if ((w.mag() != 0) && (r.mag() != 0))
  {
    ...
    r2.normalize();
    w2.normalize();
    if (r2.dot(w2) != 1) handled = turn(r, w);
  }
  return handled;
}
...
PVector turn(PVector r, PVector w)
{
  float dw = PI/8;
  int divisions;
  if (w.mag() > dw) divisions = ceil(w.mag() / dw);
  else                    divisions = 1;
  ...
  for (int i=1; i<=divisions; i++) rprime = dturn(rprime, phi);
...
PVector dturn(PVector rprime, PVector phi)
{
  ...
  PVector ua = a;
  ua.normalize();
  PVector b = PVector.mult(PVector.sub(rprime,PVector.mult(ua, ua.dot(rprime) ) ),    -1);
