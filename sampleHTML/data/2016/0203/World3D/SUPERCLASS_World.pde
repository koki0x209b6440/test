...
class WorldBox {
  ...
  WorldBox(float ix1, float iy1, float iz1,float iw, float ih,float id, float framerate){
    ...
    grav = 800;
    objs = new Object3D[1];
    objs[0] = new BasicBall3D(this, x1+w/2, y1+h/2, z1+d/2, 20, color(100, 80, 240));
  ...
  void mousework()
  {
    ...
    for(i=0; i<objs.length; i++)
    {
      ...
      if(!isWorkfinish) isWorkfinish = objs[i].mouseop(mouse3DPos.x, mouse3DPos.y, mouse3DPos.z, mousePressed);
      else break;
    }
    ...
    if( !isWorkfinish && mousePressed)//((mousePressed&&!pmousePressed)? true:false)
    {
      color c = color(random(200)+30, random(200)+30, random(200)+30);
      objs = (Object3D[]) append(objs, new ColorfulBall3D(this, mouse3DPos.x, mouse3DPos.y, mouse3DPos.z, random(30)+5, c));
      //こっちはmouseopはボールが停止してくれなくなる機能
      //「マウスがボールを掴む」ことに関係しているのは間違いないが、この挙動はどういうことだ
      objs[objs.length-1].mouseop(mouse3DPos.x, mouse3DPos.y, mouse3DPos.z,mousePressed);
    } 
